...
import { auth, db, functions, storage } from "./firebaseConfig"; // Removed database
import {
  ref as storageRef, // Renamed to avoid conflict with RTDB ref
  uploadBytes,
  getDownloadURL,
...
import { fetchPublicHolidaysForYear, isDatePublicHoliday } from "./publicHolidays"; // Added
...
export const searchServices = async ({ category = "", searchTerm = "", serviceDate = "" }) => {
  ...
  try {
    ...
    if (category) {
      servicesQuery = firestoreQuery(servicesCollection, where("category", "==", category));
    } else {
      ...
      if (!searchTerm && !serviceDate) { // Only throw if no category AND no other search criteria
        ...
        servicesQuery = firestoreQuery(servicesCollection); // Fetch all
      } else {
        servicesQuery = firestoreQuery(servicesCollection); // Fetch all, will be filtered client-side
      }
    }
  ...
  if (serviceDate) { // serviceDate is expected in "YYYY-MM-DD" format
    try {
      ...
      for (const service of filteredServices) {
        ...
        if (!service[availabilityFlag]) { // Check the dynamic field, e.g., service.isAvailableOnMonday
          continue;
        }

        // Check 2: Is it a public holiday?
        const isHoliday = await isDatePublicHoliday(serviceDate); // No need to pass publicHolidays array
        if (isHoliday) {
...
export const getAllUserSellerServiceSummaries = async () => {
  ...
  let metadataSnap;
  try {
    metadataSnap = await getDoc(metadataRef);
  } catch (error) {
  ...
  for (let i = 0; i <= lastGroupIndex; i++) {
    const groupDocRef = doc(db, "users", user.uid, "userSellerServices", String(i));
    let groupDocSnap;
    try {
      groupDocSnap = await getDoc(groupDocRef);
    } catch (error) {
...
export const getService = async (serviceId) => {
  ...
  const currentServiceReviewsQuery = firestoreQuery(
    collection(db, "reviews"),
    where("serviceId", "==", serviceId)
  );
  const currentServiceReviewsSnap = await getDocs(currentServiceReviewsQuery);
  const currentServiceReviews = currentServiceReviewsSnap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  ...
  if (serviceDataFromSnap.userId) {
    const userServicesQuery = firestoreQuery(
      collection(db, "services"),
      where("userId", "==", serviceDataFromSnap.userId)
    );
    ...
    for (const serviceDoc of userServicesSnap.docs) {
      ...
      const reviewsQuery = firestoreQuery(
        collection(db, "reviews"),
        where("serviceId", "==", relatedServiceId)
      );
...
export const uploadServiceImages = async (files, userId) => {
  const urls = [];

  console.log('userId', userId)

  for (const file of files) {
    const serviceImageRef = storageRef(storage, `services/${userId}/${Date.now()}_${file.name}`);
    const snapshot = await uploadBytes(serviceImageRef, file);
...
export const isReviewEligible = async (userId, serviceId) => {
  try {
    if (!userId || !serviceId) {
      // Alternatively, throw new Error("UserId and ServiceId are required.");
      return { eligible: false, reason: "MISSING_PARAMETERS" };
    }
    ...
    if (ordersSnapshot.empty) {
      return { eligible: false, reason: "NOT_ORDERED_OR_COMPLETED" };
    }

    const orderData = ordersSnapshot.docs[0].data();

    if (!orderData.completedAt) {
      return { eligible: false, reason: "MISSING_COMPLETION_DATE" };
    }
    ...
    return { eligible: false, reason: "INTERNAL_ERROR", error: error.message };
  }
};
...
export const confirmOrder = async (orderId) => {
  return callFirebaseFunction("approveOrder", { orderId });
};

export const rejectOrder = async (orderId, rejectionReason) => {
  return callFirebaseFunction("rejectOrder", { orderId, rejectionReason });
};

export const hasUserOrderedServiceDirectly = async (serviceId, buyerId) => {
...
export const getSellerOrders = async () => {
  try {
    ...
    if (serviceIds.length === 0) {
      return { orders: [] }; // No services = no orders
    }

    // Firestore 'in' queries support max 10 items — split if needed
    const chunkedServiceIds = serviceIds.length <= 10
      ? [serviceIds]
      : [...Array(Math.ceil(serviceIds.length / 10))].map((_, i) =>
        serviceIds.slice(i * 10, i * 10 + 10)
      );
...
export const getSellerData = async () => {
  ...
  try {
    // Count Services
    const servicesQueryFS = firestoreQuery(
      collection(db, "services"),
      where("userId", "==", userId)
    );
    const servicesSnapshot = await getDocs(servicesQueryFS);
    ...
    const completedOrdersQuery = firestoreQuery(
      collection(db, "orders"),
      where("isCompleted", "==", true),
      // Assuming 'service.userId' was intended to be a direct field on the order or requires a join/lookup.
    ...
    const getRevenue = async (startDate) => {
      const revenueQuery = firestoreQuery(
        collection(db, "orders"),
        where("isCompleted", "==", true),
        // where("sellerId", "==", userId), // Assuming 'sellerId' on order as example
        where("service.userId", "==", userId),
